...
class AllScenarios {
  ...
  static final _completeWorkflow = Scenario(
    ...
    description: 'Flow lengkap: guest booking → store admin terima device → diagnosa → selesai',
    steps: [
      ScenarioStep(
        id: 'create-order',
        description: 'Buat order baru sebagai guest (akan di-capture order_id-nya)',
        action: SchematicAction(
          type: 'invoke', target: 'guest',
          body: {
            'action': 'create-order',
            'store_id': '175ea16e-7128-42c6-b4cd-c021e67f11ef',
            'device_type': 'android', 'brand': 'Xiaomi', 'device_model': '14T',
            'delivery_method': 'walk_in',
            'customer_name': 'Siti Rahmawati', 'phone_number': '081377788899',
            'items': [{'service_type': 'screen_replacement', 'complaint': 'LCD rusak setelah jatuh',
              'sparepart_id': 'f57dcc6f-a156-446f-87bc-5e3166c93666', 'item_price': 950000}],
          },
        ),
        asserts: [
          Assertion(type: 'equals', path: 'success', expected: true),
      ...
      ScenarioStep(
        ...
        description: 'Login store admin Budi (TechFix Center)',
        action: SchematicAction(type: 'auth_login', target: '',
      ...
      ScenarioStep(
        id: 'receive-device',
        description: 'Terima device (waiting_device → device_received)',
        action: SchematicAction(type: 'invoke', target: 'orders',
          body: {'action': 'status', 'order_id': '{order_id}', 'status': 'device_received'}),
        asserts: [Assertion(type: 'equals', path: 'success', expected: true)],
      ),
      ScenarioStep(
        id: 'set-diagnosing',
        description: 'Set diagnosing (device_received → diagnosing)',
        action: SchematicAction(type: 'invoke', target: 'orders',
          body: {'action': 'status', 'order_id': '{order_id}', 'status': 'diagnosing'}),
        asserts: [Assertion(type: 'equals', path: 'success', expected: true)],
      ),
      ScenarioStep(
        id: 'submit-diagnosis',
        description: 'Submit diagnosis → waiting_approval',
        action: SchematicAction(type: 'invoke', target: 'orders',
          body: {'action': 'diagnosis', 'order_id': '{order_id}',
            'diagnosis_note': 'LCD Xiaomi 14T retak total, perlu diganti. Tidak ada kerusakan lain.',
            'service_fee': 50000,
            'items': [{'id': 'item_1', 'sparepart_id': 'f57dcc6f-a156-446f-87bc-5e3166c93666', 'final_item_price': 950000}]}),
        asserts: [Assertion(type: 'equals', path: 'success', expected: true)],
      ),
      ScenarioStep(
        id: 'verify-db-state',
        description: 'Verifikasi final di database',
        action: SchematicAction(type: 'admin_table', target: '',
