...
class _AdminDashboardScreenState extends ConsumerState<AdminDashboardScreen> {
  ...
  List<String> _validate() {
    ...
    if (_storePhone.text.trim().length < 8) {
      errors.add('No HP Toko minimal 8 digit');
    }

    if (_adminName.text.trim().isEmpty) {
      errors.add('Nama Admin Toko wajib diisi');
    }

    if (_adminPhone.text.trim().isEmpty) errors.add('No HP Admin wajib diisi');
    if (_adminPhone.text.trim().length < 8) {
      errors.add('No HP Admin minimal 8 digit');
    }
  ...
  String get _previewAddress =>
      _addressKey.currentState?.addressString ?? '(belum diisi)';
    ...
    try {
      ...
      if (e.toString().contains('DioException')) {
        ...
        if (match != null) {
          ...
          msg = 'Gagal: cek isi form atau hubungi admin.';
        }
      } else {
        msg = 'Gagal: $e';
      }
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      TextField(
                          ...
                          decoration: const InputDecoration(
                              labelText: 'Nama Toko', isDense: true)),
                      const SizedBox(height: 16),
                      ...
                      TextField(
                          ...
                          decoration: const InputDecoration(
                              labelText: 'Nama Admin Toko', isDense: true)),
                      const SizedBox(height: 12),
                      TextField(
                          controller: _adminPhone,
                          keyboardType: TextInputType.phone,
                          decoration: const InputDecoration(
                              labelText: 'No HP Admin (untuk login)',
                              prefixText: '08',
                              isDense: true)),
                      const SizedBox(height: 12),
                      TextField(
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _previewRow(
                                  'HP Toko', '08${_storePhone.text.trim()}'),
                              _previewRow('Admin', _adminName.text.trim()),
                              _previewRow(
                                  'HP Admin', '08${_adminPhone.text.trim()}'),
                              _previewRow('Perangkat',
                                  '${_android ? "Android " : ""}${_ios ? "iOS" : ""}'),
                            ],
                          ),
                        ),
                      ],
                              ...
                              children: [
                                ...
                                if (store.admins.isNotEmpty)
                                  Padding(
                                    ...
                                    child: Text(
                                        'Admin: ${store.admins.map((a) => a['fullName']).join(', ')}',
                                        style: theme.textTheme.bodySmall
                                            ?.copyWith(color: Colors.grey)),
                                  ),
                              ]),
