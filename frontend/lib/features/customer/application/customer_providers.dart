...
final customerAuthRepositoryProvider = Provider<CustomerAuthRepository>((ref) => CustomerAuthRepository(ref.watch(customerApiClientProvider), ref.watch(customerSessionProvider)));
final storeDiscoveryRepositoryProvider = Provider<StoreDiscoveryRepository>((ref) => StoreDiscoveryRepository(ref.watch(customerApiClientProvider)));
final orderRepositoryProvider = Provider<OrderRepository>((ref) => OrderRepository(ref.watch(customerApiClientProvider)));
final uploadRepositoryProvider = Provider<UploadRepository>((ref) => UploadRepository(ref.watch(customerApiClientProvider)));
final paymentRepositoryProvider = Provider<PaymentRepository>((ref) => PaymentRepository(ref.watch(customerApiClientProvider)));
...
class CustomerAuthNotifier extends AsyncNotifier<CustomerUser?> {
  ...
  Future<LoginResult> login(String phone, String password) async {
    state = const AsyncLoading();
    final result = await ref.read(customerAuthRepositoryProvider).login(phone, password);
    state = AsyncData(result.user);
    return result;
  }

  Future<void> changePassword(String oldPassword, String newPassword) async {
    await ref.read(customerAuthRepositoryProvider).changePassword(oldPassword, newPassword);
    final user = await ref.read(customerAuthRepositoryProvider).getMe();
    state = AsyncData(user.copyWith(isFirstLogin: false));
  }

  Future<void> updateProfile({required String fullName, String? address, String? avatarUrl}) async {
    final user = await ref.read(customerAuthRepositoryProvider).updateProfile(fullName: fullName, address: address, avatarUrl: avatarUrl);
    state = AsyncData(user);
  }
...
final storeDetailProvider = FutureProvider.family<ServiceStore, String>((ref, id) => ref.watch(storeDiscoveryRepositoryProvider).getStore(id));
final sparepartsProvider = FutureProvider.family<List<SparePart>, String>((ref, storeId) => ref.watch(storeDiscoveryRepositoryProvider).getSpareparts(storeId));
...
final customerOrdersProvider = FutureProvider.family<List<CustomerOrder>, String>((ref, group) {
  ...
  return ref.watch(orderRepositoryProvider).getMyOrders(status: status, limit: group == 'recent' ? 3 : 20);
});

final orderDetailProvider = FutureProvider.family<CustomerOrder, String>((ref, orderId) => ref.watch(orderRepositoryProvider).getOrderDetail(orderId));

final orderTrackingProvider = StreamProvider.family<CustomerOrder, String>((ref, orderId) async* {
...
final couponsProvider = FutureProvider<List<CouponReward>>((ref) => ref.watch(reviewRepositoryProvider).getCoupons());
final notificationsProvider = FutureProvider<List<NotificationItem>>((ref) => ref.watch(notificationRepositoryProvider).getNotifications());
final notificationPreferenceProvider = FutureProvider<bool>((ref) => ref.watch(customerSessionProvider).readNotificationPreference());
