...
class _SparepartFormScreenState extends ConsumerState<SparepartFormScreen> {
  ...
  Widget _buildBrandField(List<String> brands) {
    ...
    if (_selectedBrand != null && !allBrands.contains(_selectedBrand)) allBrands.insert(0, _selectedBrand!);
    return Row(children: [
      Expanded(
        child: DropdownButtonFormField<String>(
          ...
          onChanged: (v) => setState(() { _selectedBrand = v; _selectedDeviceModel = null; }),
        ),
      ),
      const SizedBox(width: 8),
      IconButton(icon: const Icon(Icons.add_circle_outline), onPressed: () => _showAddDialog('Brand', _newBrandController, (val) { setState(() => _selectedBrand = val); ref.invalidate(brandsProvider); })),
    ]);
  }

  Widget _buildModelField(List<String> models) {
    final allModels = [...models];
    if (_selectedDeviceModel != null && !allModels.contains(_selectedDeviceModel)) allModels.insert(0, _selectedDeviceModel!);
    return Row(children: [
      Expanded(
        child: DropdownButtonFormField<String>(
      ...
      const SizedBox(width: 8),
      IconButton(icon: const Icon(Icons.add_circle_outline), onPressed: _selectedBrand == null ? null : () => _showAddDialog('Model Device', _newModelController, (val) { setState(() => _selectedDeviceModel = val); ref.invalidate(deviceModelsProvider(_selectedBrand)); })),
    ]);
  }

  void _showAddDialog(String title, TextEditingController controller, Function(String) onAdd) {
    controller.clear();
    showDialog(context: context, builder: (c) => AlertDialog(title: Text('Tambah $title'), content: TextField(controller: controller, autofocus: true, decoration: InputDecoration(hintText: 'Nama $title')), actions: [
      TextButton(onPressed: () => Navigator.pop(c), child: const Text('Batal')),
      FilledButton(onPressed: () { final val = controller.text.trim(); if (val.isNotEmpty) { onAdd(val); Navigator.pop(c); } }, child: const Text('Tambah')),
    ]));
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(inventoryProvider.notifier).save({
        'brand': _selectedBrand!, 'deviceModel': _selectedDeviceModel!, 'partType': _selectedPartType,
        'partName': _partName.text, 'price': num.tryParse(_price.text) ?? 0, 'qty': int.tryParse(_qty.text) ?? 0,
      }, id: widget.item?.id);
